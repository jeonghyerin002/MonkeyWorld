using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class DissolveController : MonoBehaviour
{
    public Material dissolveMaterial;
    public float duration = 2f;

    void Start()
    {
        dissolveMaterial = GetComponent<Renderer>().material;
        StartCoroutine(ChangeAmountOverTime());
    }
    IEnumerator ChangeAmountOverTime()
    {
        float elapsed = 0f;                        //경과 시간
        float startValue = -1f;                    //시작 값
        float endValue = 1f;                           //끝 값

        while (elapsed < duration)            //duration 동안 반복
        {
            elapsed += Time.deltaTime;             //매 프레임 경과 시간 추가
            float t = Mathf.Clamp01(elapsed / duration);         //0~1 사이 배율로 전환
            float value = Mathf.Lerp(startValue, endValue, t);       //선형 보간으로 값 계산(-1 -> 1)
            dissolveMaterial.SetFloat("_Amount", value);          //Material의 Amount 속성에 값 적용
            yield return null;                    //다음 프레임까지 대기
        }

        dissolveMaterial.SetFloat("_Amount", endValue);            //마지막으로 끝 값을 확실하게 적용
    }

}
